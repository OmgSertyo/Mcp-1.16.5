...
public class ChatScreen extends Screen
{
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button)
    {
        ...
        else
        {
            ...
            Vec2i scale = Main.getInstance().getScaleMath().getMouse((int) mouseX, (int) mouseY, Minecraft.getInstance().getMainWindow());
            DragManager.getDraggables().values().forEach((dragging) -> {
                if (dragging.getModule().isEnabled()) {
                    dragging.onClick(scale.getX(), scale.getY(), button);
                }
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks)
    {
        ...
        Vec2i scale = Main.getInstance().getScaleMath().getMouse(mouseX, mouseY, Minecraft.getInstance().getMainWindow());
        DragManager.getDraggables().values().forEach((dragging) -> {
            if (dragging.getModule().isEnabled()) {
