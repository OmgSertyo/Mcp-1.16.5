...
public class GameRenderer implements IResourceManagerReloadListener, AutoCloseable
{
    ...
    public void displayItemActivation(ItemStack stack) {
        EventOverlay overlaysRender = new EventOverlay(EventOverlay.OverlayType.TOTEM_ANIMATION);
        EventManager.call(overlaysRender);

        if (overlaysRender.isCancelled()) {
            return;
        }
    ...
    public void setupOverlayRendering(int scale) {
        ...
        GlStateManager.ortho(0.0D, mw.getScaledWidth(), mw.getScaledHeight(), 0.0D, 1000.0D, 3000.0D);
